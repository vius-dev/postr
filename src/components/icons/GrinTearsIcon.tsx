...
export default function GrinTearsIcon({ size = 20, style, active = false }: GrinTearsIconProps) {
    const { theme } = useTheme();

    // Active colors (Original)
    const activeColors = {
        face: "#FFCC4D",
        features: "#664500",
        tears: "#5DADEC",
    ...
    // Inactive colors (Muted/Gray)
    const inactiveColors = {
        face: theme.textTertiary,
        features: theme.background, // Match background for "transparent" look or White/Black depending on contrast
        tears: theme.textTertiary,
        teeth: theme.background
    };

    const colors = active ? activeColors : inactiveColors;
